...
import { db, hasDatabase, query, recordServiceHeartbeat, transaction } from "../lib/db";
import { finalizeLaunch, markLaunchFailed, restoreGameAfterTerminalLaunch } from "../lib/launches";
  ...
  await transaction(async (client) => {
    ...
    for (const row of expired.rows) {
      if (row.free_credit_id) {
        await client.query(
          "UPDATE free_launch_credits SET status='AVAILABLE',reserved_wallet_address=NULL,reservation_expires_at=NULL,updated_at=now() WHERE id=$1 AND status='RESERVED'",
          [row.free_credit_id],
        );
      }
...
async function recordHeartbeat() {
  await Promise.allSettled([
    recordServiceHeartbeat("worker", workerId, { queue: hasRedis() ? "redis" : "postgres" }),
    recordWorkerHeartbeat(workerId),
  ]);
}

async function heartbeatLoop() {
  // Schedule the next write only after this one settles so a slow dependency
  // cannot create overlapping heartbeat calls.
  while (true) {
    await delay(10_000);
    const startedAt = Date.now();
    await recordHeartbeat();
    if (Date.now() - startedAt > 10_000) {
      console.warn("worker_heartbeat_slow", { workerId, durationMs: Date.now() - startedAt });
    }
  }
}
