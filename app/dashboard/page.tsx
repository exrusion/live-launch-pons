...
export default async function DashboardPage() {
  ...
  return <main className="shell page-shell dashboard-page"><div className="page-heading"><div><span className="eyebrow">Creator dashboard</span><h1>{session.user.xId ? `Good to see you, @${session.user.xUsername}.` : "Your game studio."}</h1><p>Draft, verify, and publish each playable launch from one place.</p></div><Link className="button button-primary" href="/create">＋ New game</Link></div>
    <div className="dashboard-summary"><article className="credit-card"><div><span>{session.user.xId ? "First launch credit" : "Optional X credit"}</span><StatusBadge status={credit?.status || "UNAVAILABLE"} /></div><strong>{credit?.status === "AVAILABLE" ? "1 free sponsored launch" : credit?.status === "USED" ? "Credit used" : session.user.xId ? "Credit pending" : "Connect X only for the credit"}</strong><p>{credit?.status === "AVAILABLE" ? "Use it at launch and the platform wallet pays the Pons fee and network gas." : session.user.xId ? "The credit is permanently tied to your verified X account ID." : "Your wallet flow works without X. X is only needed to claim the one-time free sponsored launch."}</p></article><article><span>Game drafts</span><strong>{games.filter((game) => game.status !== "LIVE").length}</strong><p>Playable versions not yet on-chain.</p></article><article><span>Live games</span><strong>{live}</strong><p>Confirmed through the official pons factory.</p></article><article><span>Verified wallet</span><strong className="address-value">{credit?.primary_wallet ? `${credit.primary_wallet.slice(0,6)}…${credit.primary_wallet.slice(-4)}` : "None"}</strong><p>Verify from the wallet control before launch.</p></article></div>
