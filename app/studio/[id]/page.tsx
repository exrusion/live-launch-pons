...
export default async function StudioPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const result = await query(
    `SELECT g.*,v.id AS version_id,v.deterministic_id,v.version_number,v.config,v.config_hash,v.manifest_hash,
      ...
      p.id AS launch_id,p.status AS launch_status,p.transaction_hash,p.error_detail,p.free_credit_id,p.sponsored_launch,p.quote_expires_at::text AS quote_expires_at,p.rebate_status,
  ...
  return <main className="shell page-shell studio-page"><div className="studio-top"><div><Link className="back-link" href="/dashboard">← Dashboard</Link><div className="title-line"><h1>{game.name}</h1><span>${game.ticker}</span><StatusBadge status={game.launch_status || game.status} /></div><p>{game.description}</p></div>{game.token_address && <Link className="button button-primary" href={`/game/${game.token_address}`}>Open live game</Link>}</div>
    <div className="studio-grid"><div><GameFrame title={game.name} publicId={game.token_address || game.slug} versionId={game.version_id} /><div className="version-card"><div><span>Frozen game version</span><b>v{game.version_number} · {game.deterministic_id}</b></div><div><span>Configuration hash</span><code>{game.config_hash}</code></div><div><span>Manifest hash</span><code>{game.manifest_hash}</code></div></div></div><LaunchPanel key={game.id} gameId={game.id} creditStatus={credit?.status} sponsorReady={sponsorReady} verifiedWalletAddress={credit?.primary_wallet || null} initialLaunch={game.launch_id ? { id: game.launch_id, status: game.launch_status, transactionHash: game.transaction_hash, tokenAddress: game.token_address, errorDetail: game.error_detail, tradingUrl: game.trading_url || (game.token_address ? `https://www.ponsfamily.com/launchpad/${game.token_address}` : null), quoteExpiresAt: game.quote_expires_at, freeCreditRequested: Boolean(game.free_credit_id), sponsoredLaunch: Boolean(game.sponsored_launch), rebateStatus: game.rebate_status, rebateTxHash: game.rebate_tx_hash, rebateErrorDetail: game.rebate_error_detail } : null} /></div>
    ...
    {!game.sponsored_launch && game.rebate_status === "FAILED" && <div className="failure-card"><b>Launch confirmed · reimbursement needs attention</b><p>{game.rebate_error_detail || game.rebate_error_code || "The reimbursement did not complete. The failure is recorded for operator review."}</p>{game.rebate_tx_hash && <a className="tx-link" href={`https://robinhoodchain.blockscout.com/tx/${game.rebate_tx_hash}`} target="_blank" rel="noreferrer">View reimbursement transaction ↗</a>}</div>}
    {!game.sponsored_launch && ["PENDING","SENDING"].includes(game.rebate_status) && <div className="credit-warning"><b>Reimbursement pending</b><p>Your launch is confirmed. The exact pons launch fee reimbursement is recorded and will be processed by the worker.</p></div>}
  </main>;
}
