...
export default async function StudioPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const result = await query(
    `SELECT g.*,v.id AS version_id,v.deterministic_id,v.version_number,v.config,v.config_hash,v.manifest_hash,
      ...
      p.id AS launch_id,p.status AS launch_status,p.transaction_hash,p.error_detail,p.rebate_status,
      p.rebate_tx_hash,p.rebate_error_code,p.rebate_error_detail,t.address AS token_address
     FROM games g JOIN game_versions v ON v.id=g.current_version_id
     LEFT JOIN LATERAL (SELECT * FROM pons_launches WHERE game_id=g.id ORDER BY created_at DESC LIMIT 1) p ON true
  ...
  return <main className="shell page-shell studio-page"><div className="studio-top"><div><Link className="back-link" href="/dashboard">← Dashboard</Link><div className="title-line"><h1>{game.name}</h1><span>${game.ticker}</span><StatusBadge status={game.launch_status || game.status} /></div><p>{game.description}</p></div>{game.token_address && <Link className="button button-primary" href={`/game/${game.token_address}`}>Open live game</Link>}</div>
    <div className="studio-grid"><div><GameFrame title={game.name} publicId={game.token_address || game.slug} versionId={game.version_id} /><div className="version-card"><div><span>Frozen game version</span><b>v{game.version_number} · {game.deterministic_id}</b></div><div><span>Configuration hash</span><code>{game.config_hash}</code></div><div><span>Manifest hash</span><code>{game.manifest_hash}</code></div></div></div><LaunchPanel gameId={game.id} creditStatus={credit?.status} sponsorReady={sponsorReady} /></div>
