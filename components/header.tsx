  ...
  return <header className="site-header"><div className="shell header-inner">
    ...
    <nav className={menu ? "site-nav nav-open" : "site-nav"} aria-label="Main navigation">
      ...
      {status !== "loading" && (session
        ...
        : <button className="mobile-nav-auth" disabled={xEnabled !== true} onClick={() => signIn("twitter", { callbackUrl: "/dashboard" })}>{xEnabled === null ? "Checking X…" : xEnabled ? "Sign in with X" : "X setup pending"}</button>)}
    </nav>
    <div className="header-actions">
      <WalletButton />
      {status !== "loading" && (session ? <Link className="account-button" href="/dashboard"><span>{session.user?.image ? <img src={session.user.image} alt="" /> : "@"}</span><b>@{session.user?.xUsername || "creator"}</b></Link> : <button className="button button-primary button-small" disabled={xEnabled !== true} title={xEnabled === false ? "X OAuth credentials are required" : undefined} onClick={() => signIn("twitter", { callbackUrl: "/dashboard" })}>{xEnabled === null ? "Checking X…" : xEnabled ? "Sign in with X" : "X setup pending"}</button>)}
