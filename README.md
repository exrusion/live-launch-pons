...
The app never stores creator wallet keys. Normal paid launches are signed by the connected creator wallet. A one-time free launch is signed and paid by the isolated worker sponsor wallet while the verified creator wallet remains the creator-fee recipient. Credits are consumed only after a verified `TokenLaunched` receipt.

## Sponsored free-launch safety

For the free first launch, the web service validates and reserves the exact Pons call, then the worker signs and submits it from the owner-funded sponsor wallet. Configure `SPONSOR_ADDRESS`, `MAX_FREE_LAUNCH_REBATE_WEI`, and `FREE_LAUNCH_DAILY_BUDGET_WEI` on the web service for funding and liability preflights. Configure the same values plus `SPONSOR_PRIVATE_KEY` only on the worker. The derived key address must match `SPONSOR_ADDRESS`.

The sponsored launch hash and raw transaction are persisted before broadcast. Retries rebroadcast that same transaction, preventing duplicate launches after a process crash. Legacy reimbursements remain recoverable by the worker. Terminal failures restore an unused credit and remain visible with an audit trail. Disable `FREE_LAUNCH_REBATE_ENABLED` to stop new sponsored launches without losing recorded commitments.
...
- Owner-required for sponsored free launches: `SPONSOR_ADDRESS` on web and worker, `SPONSOR_PRIVATE_KEY` on the worker only, funded Robinhood Chain ETH, per-launch/daily caps, then `FREE_LAUNCH_REBATE_ENABLED=true` on both services.
